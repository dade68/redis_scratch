...
AVLNode *avl_fix(AVLNode *node)
{
    while (true)
    {
        ...
        if (node->parent)
        {
            from = (node == node->parent->left) ? &node->parent->left : &node->parent->right;
        }
        if (l == r + 2)
        {
...
AVLNode *avl_del(AVLNode *node)
{
    if (node->right == NULL)
    {
        ...
        if (parent)
        {
            // atatch the left subtree to the parent
...
// note: the worse-case is O(log(n)) regardless of how long the offset is
